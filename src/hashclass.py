class HashTable(object):
    ...
    def add(self, key, value):
        ...
        if self.array[index] is not None:
            ...
            for kvp in self.array[index]:
                ...
                print('Matched the key')
                if kvp[0] == key:
                    print('Matched the key Inside the if length of kvp is {}'.format(len(kvp)))
                    self.array[index].append([key,value])
                    print(self.array[index])
                    '''for j in range(2,len(kvp)):
                        print('j value is {}'.format(j))
                        if kvp[j] is not None:
                            kvp[j] = value
                            print('Matched the key Inside For loop for {}'.format(kvp[j]))'''
                    break
                    ##kvp[1] = value
                    ##self.array[index + 1].append([key, value])   ##Added to add the existing key and vals
                    #break
            else:
    ...
    def get(self, key):
        ...
        if self.array[index] is None:
            ...
            for kvp in self.array[index]:
                if kvp[0] == key:
                    print('all the values {},{}self.array[index]'.format(kvp,self.array[index]))
                    return self.array[index]
                    '''return kvp[1]'''
            
                '''if kvp[0] == key:
                    for j in range(1,len(kvp)):
                        if kvp[j] is not None:
                            t_li.append(kvp[j])
                    return t_li  ##kvp[1]'''
